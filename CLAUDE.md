...
`src/data/snapshots/exporter.py` is the **only** interface other repos use — they never import this codebase. `v1.0.0` tag on master pins the contract; bump major on breaking Parquet-shape changes. Repo: [`knesset-platform`](https://github.com/AT020993/knesset-platform).
...
Multi-user bill classification for democratic erosion research. **Full docs**: [`src/ui/services/cap/CLAUDE.md`](src/ui/services/cap/CLAUDE.md)
...
| **Scripts** | `scripts/import_research_coding.py`, `import_government_bills.py`, `import_filtered_coding.py`, `import_coalition_data.py`, `export_uncoded_private_bills.py`, `export_uncoded_committee_gov_bills.py`, `upload_to_gcs.py`, `download_from_gcs.py`, `diagnose_db.py` |
| **Recurring Bills** | `src/data/recurring_bills/` — Tal Alovitz ingestion → `bill_classifications` table + `data/snapshots/bill_classifications.parquet` + `v_cap_bills_with_recurrence` view |
...
`ARCHITECTURE.md` · `ARCHITECTURE_DIAGRAM.md` · `DEPLOYMENT_GUIDE.md` · `DEPLOYMENT_OPTIONS.md` · `RESEARCHER_GUIDE.md` · `PERFORMANCE_OPTIMIZATIONS.md` · `docs/CAP_ANNOTATION_GUIDE.md` · `docs/FULL_DATASET_DOWNLOAD.md`

## Claude Code Automations

- `.claude/settings.json` — Hooks: auto-format (black/isort), SQL injection detection, .env edit blocking
- `.claude/agents/` — Subagent definitions: `code-reviewer.md`, `cap-debugger.md`, `data-refresh.md`
- `.claude/skills/cap-annotate/` — CAP annotation utilities
- `.claude/reviews/` — Code review results from manual agent runs
