...
class DataRefreshPageRenderer:
    ...
    def _render_query_results_display(self) -> None:
        ...
        if not display_df.empty:
            ...
            with col_csv:
                ...
                st.download_button("⬇️ CSV", csv_data, f"{safe_name}_results.csv", "text/csv", key=f"csv_dl_{safe_name}")
    ...
    def render_table_explorer_section(self) -> None:
        ...
        if (
            SessionStateManager.get_show_table_explorer_results()
            and SessionStateManager.get_executed_table_explorer_name()
        ):
    ...
    def _render_table_explorer_display(self) -> None:
        ...
        if not results_df.empty:
            ...
            with col_csv_ex:
                ...
                st.download_button("⬇️ CSV", csv_data, f"{safe_name}_data.csv", "text/csv", key=f"csv_dl_ex_{safe_name}")
    ...
    def render_ad_hoc_sql_section(self, connect_func: Callable) -> None:
        ...
        with st.expander("🧑‍🔬 Run an Ad-hoc SQL Query (Advanced)", expanded=False):
            ...
            st.markdown(
                "Construct your SQL query. Sidebar filters are **not** automatically applied here. Include them in your `WHERE` clause if needed."
            )
            ...
            sql_query_input = st.text_area(
                "Enter your SQL query:", default_sql_query, height=150, key="adhoc_sql_query"
            )
    ...
    def _execute_ad_hoc_query(self, sql_query: str, connect_func: Callable) -> None:
        ...
        try:
            ...
            if not adhoc_result_df.empty:
                ...
                st.download_button("⬇️ CSV", csv_data, "adhoc_results.csv", "text/csv", key="adhoc_csv_dl")
