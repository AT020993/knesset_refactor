...
class PlotsPageRenderer:
    ...
    def _render_topic_selection(self, available_plots: Dict[str, Dict[str, Callable]]) -> str:
        ...
        topic_select_default_index = (
            plot_topic_options.index(current_selected_topic) if current_selected_topic in plot_topic_options else 0
        )
    ...
    def _render_chart_selection(self, available_plots: Dict[str, Dict[str, Callable]], selected_topic: str) -> str:
        """
    ...
    def _render_plot_options(self, selected_chart: str, knesset_options: List[str]) -> None:
        ...
        if selected_chart in ["Queries by Time Period", "Agenda Items by Time Period"]:
            self._render_time_period_plot_options(selected_chart, plot_knesset_options, current_main_knesset_selection)
        else:
    ...
    def _render_time_period_plot_options(
        ...
        knesset_select_default_index = (
            plot_knesset_options.index(current_selection) if current_selection in plot_knesset_options else 0
        )
    ...
    def _render_single_knesset_plot_options(
        ...
        options_for_single_knesset_plot = [
            opt for opt in plot_knesset_options if opt != "All Knessets (Color Coded)" and opt != ""
        ]
        ...
        single_knesset_default_idx = (
            effective_options_single.index(current_selection) if current_selection in effective_options_single else 0
        )
    ...
    def _render_query_filters(self, selected_chart: str) -> None:
        ...
        with col1:
            # Query Type filter
            query_types = st.session_state.get(
                "available_query_types",
                ["שאילתה", "בקשה לקדימות דיון", "שאילתה לשם דיון בכנסת", "הצעה לסדר היום", "הודעה על בעיית התרחשות"],
            )
        ...
        with col2:
            # Query Status filter
            query_statuses = st.session_state.get(
                "available_query_statuses", ["הוגשה", "נתקבלה תשובה", "טרם נתקבלה תשובה", "נדחתה"]
            )
    ...
    def _render_agenda_filters(self, selected_chart: str) -> None:
        ...
        with col1:
            ...
            session_types = st.session_state.get("available_session_types", ["מליאה", "ועדה", "ועדת משנה"])
        ...
        with col2:
            ...
            agenda_statuses = st.session_state.get("available_agenda_statuses", ["קיים", "בוטל", "נדחה"])
    ...
    def _populate_filter_options(self) -> None:
        ...
        try:
            ...
            with get_db_connection(self.db_path, read_only=True, logger_obj=self.logger) as con:
                # Query types
                query_types_query = (
                    "SELECT DISTINCT TypeDesc FROM KNS_Query WHERE TypeDesc IS NOT NULL ORDER BY TypeDesc"
                )
                ...
                try:
                    session_types_query = (
                        "SELECT DISTINCT SubTypeDesc FROM KNS_Agenda WHERE SubTypeDesc IS NOT NULL ORDER BY SubTypeDesc"
                    )
                ...
                try:
                    bill_types_query = (
                        "SELECT DISTINCT SubTypeDesc FROM KNS_Bill WHERE SubTypeDesc IS NOT NULL ORDER BY SubTypeDesc"
                    )
    ...
    def _generate_and_display_plot(
        ...
        plot_args = self._build_plot_arguments(final_knesset_filter, faction_display_map, connect_func, selected_chart)
    ...
    def _build_plot_arguments(
        ...
        plot_args = {
            ...
            "faction_filter": [faction_display_map[name] for name in faction_filter if name in faction_display_map],
        }
