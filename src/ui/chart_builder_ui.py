...
def display_chart_builder(
    ...
    if selectbox_output_table != st.session_state.get("builder_selected_table_previous_run"):
        ...
        if selectbox_output_table and selectbox_output_table != "":
            ...
            # Fetch initial data for populating chart-specific filters, applying GLOBAL sidebar filters
            con = None
            try:
    ...
    if st.session_state.get("builder_selected_table"):
        ...
        if st.session_state.previous_builder_chart_type != st.session_state.builder_chart_type:
            logger_obj.info(f"Chart type changed from {st.session_state.previous_builder_chart_type} to {st.session_state.builder_chart_type}. Validating axes.")
        ...
        if st.button("📊 Generate Chart", key="btn_generate_custom_chart"):
            ...
            if faction_filter_cs_selected_names:
                # Rebuild map based on current cs_filter_data to ensure consistency
            ...
            if valid_input:
                ...
                try:
                    ...
                    if knesset_filter_cs_selected and 'KnessetNum' in df_for_chart.columns:
                        # Ensure KnessetNum in df_for_chart is int if knesset_filter_cs_selected contains ints
                        if not pd.api.types.is_integer_dtype(df_for_chart['KnessetNum']):
                            try: # Attempt conversion, handling potential errors if mixed types or unconvertible strings exist
                                df_for_chart['KnessetNum'] = pd.to_numeric(df_for_chart['KnessetNum'], errors='coerce').fillna(-1).astype(int) # Coerce errors to -1 or handle appropriately
                    ...
                    if faction_filter_cs_selected_ids and 'FactionID' in df_for_chart.columns:
                         # Ensure FactionID in df_for_chart is int if faction_filter_cs_selected_ids contains ints
                        if not pd.api.types.is_integer_dtype(df_for_chart['FactionID']):
                            try:
