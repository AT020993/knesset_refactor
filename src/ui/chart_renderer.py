...
class ChartRenderer:
    ...
    def render_table_selection(self) -> str:
        ...
        try:
            tables = ui_utils.get_db_table_list(self.chart_builder.db_path, self.chart_builder.logger)
            table_options = [""] + [t for t in tables if t.startswith("KNS_")]
            
            selected_table = st.selectbox(
                "1. Select Table:",
                options=table_options,
                index=0 if not st.session_state.get("builder_selected_table") else 
                      table_options.index(st.session_state.builder_selected_table) if st.session_state.builder_selected_table in table_options else 0,
    ...
    def generate_chart(self):
        ...
        try:
            ...
            df = ui_utils.execute_query_to_df(self.chart_builder.db_path, query, self.chart_builder.logger)
            ...
            if chart_type == "Bar Chart" and x_col and y_col:
                ...
                st.info("Please specify both X and Y columns for the selected chart type.")
                st.write("Available columns:", list(df.columns))
                
        except Exception as e:
            st.error(f"Error generating chart: {e}")
            self.chart_builder.logger.error(f"Chart generation error: {e}", exc_info=True)
