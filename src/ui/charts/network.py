...
class NetworkCharts(BaseChart):
        ...
        **kwargs,
        ...
        try:
            with get_db_connection(self.db_path, read_only=False, logger_obj=self.logger) as con:
                ...
                if df.empty:
                    st.info(
                        f"No MK collaboration data found for '{filters['knesset_title']}' with minimum {min_collaborations} collaborations."
                    )
                    return None
        ...
        **kwargs,
        ...
        try:
            with get_db_connection(self.db_path, read_only=False, logger_obj=self.logger) as con:
                ...
                if df.empty:
                    st.info(
                        f"No faction collaboration data found for '{filters['knesset_title']}' with minimum {min_collaborations} collaborations."
                    )
                    return None
        ...
        **kwargs,
        ...
        try:
            with get_db_connection(self.db_path, read_only=False, logger_obj=self.logger) as con:
                ...
                if df.empty:
                    st.info(
                        f"No faction collaboration breakdown data found for '{filters['knesset_title']}' with minimum {min_collaborations} collaborations."
                    )
                    return None
    ...
    def _create_network_chart(
        ...
        main_nodes = main_nodes.rename(
            columns={edge_source_col: node_id_col, main_name_col: node_name_col, main_group_col: node_group_col}
        )

        # Create supporter nodes DataFrame
        supp_nodes = df[supp_node_cols].copy()
        supp_nodes = supp_nodes.rename(
            columns={edge_target_col: node_id_col, supp_name_col: node_name_col, supp_group_col: node_group_col}
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=edge_x, y=edge_y, mode="lines", line=dict(width=1, color="#888"), hoverinfo="none", showlegend=False
            )
        )
        ...
        for group in unique_groups:
            ...
            hover_text = [
                f"{name}<br>{group}<br>Connections: {len(edges[(edges[edge_source_col] == node_id) | (edges[edge_target_col] == node_id)])}"
    ...
    def _create_coalition_network_chart(self, df: pd.DataFrame, title_suffix: str) -> go.Figure:
        ...
        coalition_pairs = (
            df.groupby(["MainCoalitionStatus", "SupporterCoalitionStatus"])["CollaborationCount"].sum().reset_index()
        )
    ...
    def _create_mk_network_chart(self, df: pd.DataFrame, title_suffix: str) -> go.Figure:
        ...
        try:
            ...
            main_nodes = df[
                ["MainInitiatorID", "MainInitiatorName", "MainInitiatorFaction", "MainInitiatorTotalBills"]
            ].copy()
        ...
        fig.update_layout(
            ...
            annotations=[],
            legend=dict(orientation="v", yanchor="top", y=1, xanchor="left", x=1.05, font=dict(size=10)),
        )
    ...
    def _create_faction_network_chart(self, df: pd.DataFrame, title_suffix: str) -> go.Figure:
        ...
        status_colors = {"Coalition": "#1f77b4", "Opposition": "#ff7f0e", "Unknown": "#808080"}
        ...
        fig.update_layout(
            ...
            plot_bgcolor="rgba(240,240,240,0.1)",
            annotations=[],
            legend=dict(orientation="v", yanchor="top", y=1, xanchor="left", x=1.05, font=dict(size=12)),
        )
    ...
    def _create_faction_breakdown_chart(self, df: pd.DataFrame, title_suffix: str) -> go.Figure:
        ...
        fig.update_layout(
            ...
            yaxis=dict(title="Political Factions", showgrid=False),
            barmode="stack",
        ...
        **kwargs,
        ...
        try:
            with get_db_connection(self.db_path, read_only=False, logger_obj=self.logger) as con:
                ...
                if df.empty:
                    st.info(
                        f"No faction collaboration chord data found for '{filters['knesset_title']}' with minimum {min_collaborations} collaborations."
                    )
                    return None
    ...
    def _create_faction_matrix_chart(self, df: pd.DataFrame, title_suffix: str, min_collaborations: int) -> go.Figure:
        ...
        pivot_data = df.pivot_table(
            index="MainFactionName", columns="SupporterFactionName", values="CollaborationCount", fill_value=0
        )
        ...
        fig = go.Figure(
            data=go.Heatmap(
                ...
                colorbar=dict(title="Collaboration Count", titleside="right", thickness=15, len=0.8),
                showscale=True,
            )
        )
        ...
        fig.update_layout(
            ...
            title_x=0.5,
            xaxis=dict(title="Supporting Factions", side="bottom", tickangle=45),
            yaxis=dict(title="Primary Initiating Factions", tickmode="linear"),
    ...
    def _create_enhanced_faction_matrix_chart(
        ...
        if collab_matrix.max() > 0:
            fig.add_trace(
                go.Heatmap(
                    ...
                    customdata=hover_text,
                    colorbar=dict(
                        title="Collaboration Count", titleside="right", thickness=15, len=0.8, x=1.02, xanchor="left"
                    ),
                    zmin=0,
        ...
        if show_solo_bills and solo_matrix.max() > 0:
            fig.add_trace(
                go.Heatmap(
                    ...
                    customdata=hover_text,
                    colorbar=dict(
                        title="Solo Bills Count", titleside="right", thickness=15, len=0.8, x=1.12, xanchor="left"
                    ),
                    zmin=0,
        ...
        fig.update_layout(
            title=title_text,
            title_x=0.5,
            xaxis=dict(title="Supporting/Target Factions", side="bottom", tickangle=45, tickfont=dict(size=10)),
            yaxis=dict(
