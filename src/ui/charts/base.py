...
from utils.query_builder import FilterOperator, SecureQueryBuilder
...
class BaseChart(ABC):
    ...
    def build_secure_filters(
        ...
        **kwargs,
        ...
        knesset_condition, is_single = builder.build_knesset_filter(knesset_filter, f"{prefix}KnessetNum")
        ...
        if faction_filter:
            filters["faction_condition"] = builder.build_faction_filter(faction_filter, "FactionName")
        else:
            filters["faction_condition"] = "1=1"

        # Add advanced filters with secure parameter binding
        filters["advanced_conditions"] = self._build_secure_advanced_filters(builder, prefix, date_column, **kwargs)
    ...
    def _build_secure_advanced_filters(
        ...
        **kwargs,
        ...
        if query_type_filter:
            conditions.append(
                builder.build_filter_condition(f"{prefix}TypeDesc", FilterOperator.IN, values=query_type_filter)
            )

        # Status filters using joined table alias
        query_status_filter = kwargs.get("query_status_filter", [])
        if query_status_filter:
            conditions.append(builder.build_filter_condition('s."Desc"', FilterOperator.IN, values=query_status_filter))

        # Agenda-specific filters
        session_type_filter = kwargs.get("session_type_filter", [])
        if session_type_filter:
            conditions.append(
                builder.build_filter_condition(f"{prefix}SubTypeDesc", FilterOperator.IN, values=session_type_filter)
            )
        ...
        if bill_type_filter:
            conditions.append(
                builder.build_filter_condition(f"{prefix}SubTypeDesc", FilterOperator.IN, values=bill_type_filter)
            )

        bill_status_filter = kwargs.get("bill_status_filter", [])
        if bill_status_filter:
            conditions.append(builder.build_filter_condition('s."Desc"', FilterOperator.IN, values=bill_status_filter))
        ...
        if start_date:
            conditions.append(
                builder.build_filter_condition(f"{prefix}{date_column}", FilterOperator.GREATER_EQUAL, start_date)
            )

        end_date = kwargs.get("end_date")
        if end_date:
            conditions.append(
                builder.build_filter_condition(f"date({prefix}{date_column})", FilterOperator.LESS_EQUAL, end_date)
            )
