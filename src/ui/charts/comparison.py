    ...
    def plot_query_status_by_faction(
        ...
        **kwargs,
        ...
        if not knesset_filter or len(knesset_filter) != 1:
            st.info("Please select a single Knesset to view this chart.")
            return None
        ...
        faction_filter_sql = ""
        if faction_filter:
            joined = ", ".join([f"'{f}'" for f in faction_filter])
            faction_filter_sql = f" AND p2p.FactionName IN ({joined})"
        ...
        try:
            with get_db_connection(
                ...
                self.logger.debug(f"Executing query_status_by_faction SQL: {query}")
                df = safe_execute_query(con, query, self.logger)

                if df.empty:
                    st.info(
                        f"No query data for Knesset {single_knesset_num} with the current filters."
                    )
                    return None
                ...
                for _, row in status_totals.iterrows():
                    ...
                    parents.append("")
                    values.append(int(row["QueryCount"]))
            ...
            self.logger.error(
                f"Error generating 'query_status_by_faction' chart: {e}",
                exc_info=True,
            )
            st.error(f"Could not generate 'Query Status by Faction' chart: {e}")
            return None
