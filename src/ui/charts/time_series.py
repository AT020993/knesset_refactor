...
class TimeSeriesCharts(BaseChart):
    ...
    def plot_queries_by_time_period(
        ...
        filters = self.build_filters(knesset_filter, faction_filter)
        ...
        try:
            with get_db_connection(self.db_path, read_only=True, logger_obj=self.logger) as con:
                ...
                time_configs = {
                    ...
                    "Quarterly": {
                        "sql": f"strftime(CAST({date_column} AS TIMESTAMP), '%Y') || '-Q' || CAST(quarter(CAST({date_column} AS TIMESTAMP)) AS VARCHAR)",
                        "label": "Year-Quarter"
                    },
                    "Yearly": {
    ...
    def plot_agendas_by_time_period(
        ...
        filters = self.build_filters(knesset_filter)
        ...
        try:
            with get_db_connection(self.db_path, read_only=True, logger_obj=self.logger) as con:
                ...
                # Similar implementation to queries but for agendas
                # This would follow the same pattern as plot_queries_by_time_period
                # but use KNS_Agenda table instead
