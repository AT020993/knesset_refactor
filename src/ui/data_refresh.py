...
if 'builder_names' not in st.session_state: st.session_state.builder_names = None # For pie charts
if 'builder_values' not in st.session_state: st.session_state.builder_values = None # For pie charts
...
if 'builder_barmode' not in st.session_state: st.session_state.builder_barmode = 'relative' # For bar charts
...
def get_table_columns(table_name: str) -> tuple[list[str], list[str], list[str]]:
    ...
    try:
        ...
        numeric_cols = columns_df[columns_df['type'].str.contains('INTEGER|FLOAT|DOUBLE|DECIMAL|NUMERIC|BIGINT|SMALLINT|TINYINT', case=False, na=False)]['name'].tolist()
        
        # Consider date/time types as categorical for many plot types, or handle them separately
        # For simplicity, non-numeric are treated as categorical here.
...
def get_filter_options_from_db():
    ...
    try:
        con = _connect(read_only=True) 
        knesset_nums_query = "SELECT DISTINCT KnessetNum FROM KNS_KnessetDates ORDER BY KnessetNum DESC;"
...
EXPORTS = {
    "Queries + Full Details": {
        ...
        """, "knesset_filter_column": "Q.KnessetNum", "faction_filter_column": "P2P.FactionID"
    },
    "Agenda Items + Full Details": {
        ...
        """, "knesset_filter_column": "A.KnessetNum", "faction_filter_column": "INIT_P2P.FactionID"
    }
}
...
def _handle_data_refresh_button_click():
    ...
    def _sidebar_progress_cb(table_name_done: str, num_rows_fetched: int):
        ...
        if 'completed_tables_count' not in st.session_state: st.session_state.completed_tables_count = 0
    ...
    try:
        ...
        if 'completed_tables_count' in st.session_state: del st.session_state.completed_tables_count
...
st.session_state.ms_tables_to_refresh = st.sidebar.multiselect(
    label="Select tables to refresh/fetch:", options=options_for_multiselect,
    default=default_selection_refresh, key="ms_tables_to_refresh_widget",
)
...
if _SELECT_ALL_TABLES_OPTION in st.session_state.ms_tables_to_refresh:
    if not st.session_state.get('all_tables_selected_for_refresh_flag', False): 
        ...
        st.session_state.all_tables_selected_for_refresh_flag = True; st.rerun() 
elif st.session_state.get('all_tables_selected_for_refresh_flag', False) and \
     _SELECT_ALL_TABLES_OPTION not in st.session_state.ms_tables_to_refresh:
    st.session_state.ms_tables_to_refresh = [] 
    st.session_state.all_tables_selected_for_refresh_flag = False; st.rerun()
...
st.session_state.selected_query_name = st.sidebar.selectbox(
    "Select a predefined query:", options=query_names_options, 
    index=query_names_options.index(st.session_state.selected_query_name) if st.session_state.selected_query_name in query_names_options else 0,
...
if st.sidebar.button("▶️ Run Selected Query", disabled=(not st.session_state.selected_query_name)):
    if st.session_state.selected_query_name and DB_PATH.exists():
        try:
            ...
            applied_filters_info = []; where_conditions = []
            ...
            if faction_filter_col and st.session_state.ms_faction_filter:
                selected_faction_ids = [faction_display_map[name] for name in st.session_state.ms_faction_filter if name in faction_display_map]
            ...
            if where_conditions:
                ...
                keyword_to_use = "AND" if re.search(r'\sWHERE\s', modified_sql, re.IGNORECASE) else "WHERE"
                filter_string_to_add = f" {keyword_to_use} {combined_where_clause}"
                clauses_keywords_to_find = [r'GROUP\s+BY', r'HAVING', r'WINDOW', r'ORDER\s+BY', r'LIMIT', r'OFFSET', r'FETCH']
                ...
                for pattern_str in clauses_keywords_to_find:
                    ...
                    if match and match.start() < insertion_point: insertion_point = match.start()
                prefix = modified_sql[:insertion_point].strip(); suffix = modified_sql[insertion_point:].strip()
            ...
            st.session_state.show_query_results = True; st.session_state.show_table_explorer_results = False 
            ...
            st.code(str(e) + "\n\n" + _format_exc()); st.session_state.show_query_results = False 
            st.session_state.query_results_df = pd.DataFrame() 
...
st.session_state.selected_table_for_explorer = st.sidebar.selectbox(
    "Select a table to explore:", options=db_tables_list_for_explorer, 
    index=db_tables_list_for_explorer.index(st.session_state.selected_table_for_explorer) if st.session_state.selected_table_for_explorer in db_tables_list_for_explorer else 0,
...
if st.sidebar.button("🔍 Explore Selected Table", disabled=(not st.session_state.selected_table_for_explorer)):
    if st.session_state.selected_table_for_explorer and DB_PATH.exists():
        ...
        try:
            ...
            join_clause = ""; select_prefix = f"\"{table_to_explore}\".*" 
            ...
            elif table_to_explore.lower() == "kns_persontoposition" and "userfactioncoalitionstatus" in db_tables_list_lower:
            ...
            if table_to_explore.lower() == "kns_faction" and join_clause: knesset_col_name_explorer = "f.KnessetNum"
            if table_to_explore.lower() == "kns_persontoposition" and join_clause: knesset_col_name_explorer = "p2p.KnessetNum"
            if "knessetnum" in table_columns and st.session_state.ms_knesset_filter: 
                where_clauses.append(f"{knesset_col_name_explorer} IN ({', '.join(map(str, st.session_state.ms_knesset_filter))})")
            
            faction_col_name_explorer = "FactionID" 
            if table_to_explore.lower() == "kns_faction" and join_clause: faction_col_name_explorer = "f.FactionID"
            if table_to_explore.lower() == "kns_persontoposition" and join_clause: faction_col_name_explorer = "p2p.FactionID"
            if "factionid" in table_columns and st.session_state.ms_faction_filter:
                selected_faction_ids_explorer = [faction_display_map[name] for name in st.session_state.ms_faction_filter if name in faction_display_map]
                if selected_faction_ids_explorer:
                    where_clauses.append(f"{faction_col_name_explorer} IN ({', '.join(map(str, selected_faction_ids_explorer))})")

            final_query = base_query
            if join_clause: final_query += f" {join_clause}"
            if where_clauses: final_query += " WHERE " + " AND ".join(where_clauses)
            
            order_by_col_explorer = None
            if "lastupdateddate" in table_columns: order_by_col_explorer = "LastUpdatedDate"
            elif "startdate" in table_columns: order_by_col_explorer = "StartDate" 
            elif table_columns: order_by_col_explorer = f"\"{table_columns[0]}\"" 
            if order_by_col_explorer:
                prefix_for_order = "f." if table_to_explore.lower() == "kns_faction" and join_clause else "p2p." if table_to_explore.lower() == "kns_persontoposition" and join_clause else ""
                final_query += f" ORDER BY {prefix_for_order}{order_by_col_explorer.replace('\"','')} DESC" if prefix_for_order and order_by_col_explorer.lower().replace('\"','') in table_columns else f" ORDER BY {order_by_col_explorer} DESC"
            ...
            st.session_state.show_table_explorer_results = True; st.session_state.show_query_results = False 
            ...
            st.code(str(e) + "\n\n" + _format_exc()); st.session_state.show_table_explorer_results = False 
            st.session_state.table_explorer_df = pd.DataFrame() 
    elif not st.session_state.selected_table_for_explorer: st.warning("Please select a table to explore.")
    elif not DB_PATH.exists(): st.error("Database not found. Cannot explore tables.")

st.sidebar.divider()
st.sidebar.header("📊 Filters (Apply to Queries, Explorer & Plots)")
st.session_state.ms_knesset_filter = st.sidebar.multiselect(
    "Knesset Number(s):", options=knesset_nums_options, 
    default=st.session_state.get('ms_knesset_filter', []), key="ms_knesset_filter_widget" 
)
st.session_state.ms_faction_filter = st.sidebar.multiselect(
    "Faction(s) (by Knesset):", options=list(faction_display_map.keys()), 
    default=st.session_state.get('ms_faction_filter', []), 
    help="Select factions. The Knesset number in parentheses provides context.", key="ms_faction_filter_widget" 
)
...
if st.session_state.show_query_results and st.session_state.executed_query_name:
    ...
    if not st.session_state.query_results_df.empty:
        ...
        with col_csv: st.download_button("⬇️ CSV", st.session_state.query_results_df.to_csv(index=False).encode('utf-8-sig'), f"{safe_name}_results.csv", "text/csv")
        with col_excel:
            excel_buffer = io.BytesIO()
            with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer: st.session_state.query_results_df.to_excel(writer, index=False, sheet_name='Results')
    ...
    else: st.info("The query returned no results with the current filters.")
    with st.expander("Show Executed SQL", expanded=False): st.code(st.session_state.get('last_executed_sql', 'No SQL executed yet.'), language='sql')
else: st.info("Run a predefined query from the sidebar to see results here.")
...
if st.session_state.show_table_explorer_results and st.session_state.executed_table_explorer_name:
    ...
    if not st.session_state.table_explorer_df.empty:
        ...
        col_csv, col_excel = st.columns(2)
        with col_csv: st.download_button("⬇️ CSV", st.session_state.table_explorer_df.to_csv(index=False).encode('utf-8-sig'), f"{safe_name}_data.csv", "text/csv")
        with col_excel:
            excel_buffer = io.BytesIO()
            with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer: st.session_state.table_explorer_df.to_excel(writer, index=False, sheet_name='TableData')
...
else: st.info("Explore a table from the sidebar to see its data here.")
...
if not DB_PATH.exists():
    ...
    if current_selected_plot not in plot_options: current_selected_plot = ""; st.session_state.selected_plot_name = ""
    st.session_state.selected_plot_name = st.selectbox("Choose a predefined visualization:", options=plot_options, index=plot_options.index(current_selected_plot), key="sb_selected_plot")
    if st.session_state.selected_plot_name:
        ...
        with st.spinner(f"Generating '{st.session_state.selected_plot_name}'..."):
            try:
                ...
                if figure: st.plotly_chart(figure, use_container_width=True); st.session_state.generated_plot_figure = figure
            except Exception as e:
                ui_logger.error(f"Error displaying plot '{st.session_state.selected_plot_name}': {e}", exc_info=True)
                st.error(f"An error occurred while generating the plot: {e}"); st.code(str(e) + "\n\n" + _format_exc())
    else: st.info("Select a predefined visualization from the dropdown to display it.")
...
if not DB_PATH.exists():
    ...
    selected_table = st.selectbox(
        ...
        index=db_tables_for_builder.index(st.session_state.builder_selected_table) if st.session_state.builder_selected_table in db_tables_for_builder else 0,
        key="builder_table_select"
    )

    if selected_table and selected_table != st.session_state.get('builder_selected_table_previous_run'):
    ...
    if st.session_state.builder_selected_table:
        ...
        st.session_state.builder_chart_type = st.selectbox(
            "2. Select Chart Type:",
            options=chart_types,
            index=chart_types.index(st.session_state.builder_chart_type) if st.session_state.builder_chart_type in chart_types else 0
        )
        ...
        with cols_c1:
            ...
            if st.session_state.builder_chart_type not in ["pie"]:
                st.session_state.builder_x_axis = st.selectbox("X-axis:", options=st.session_state.builder_columns, 
                                                                index=st.session_state.builder_columns.index(st.session_state.builder_x_axis) if st.session_state.builder_x_axis in st.session_state.builder_columns else 0)
            
            if st.session_state.builder_chart_type not in ["pie", "histogram"]: # Histogram Y is count
                 st.session_state.builder_y_axis = st.selectbox("Y-axis:", options=st.session_state.builder_numeric_columns if st.session_state.builder_chart_type != "bar" else st.session_state.builder_columns, # Bar Y can be cat or num
                                                                index=(st.session_state.builder_numeric_columns if st.session_state.builder_chart_type != "bar" else st.session_state.builder_columns).index(st.session_state.builder_y_axis) if st.session_state.builder_y_axis in (st.session_state.builder_numeric_columns if st.session_state.builder_chart_type != "bar" else st.session_state.builder_columns) else 0,
                                                                help="Select a numeric column for Y-axis (except for Bar charts which can also use categorical)." if st.session_state.builder_chart_type != "bar" else "Select column for Y-axis.")


            if st.session_state.builder_chart_type == "pie":
                st.session_state.builder_names = st.selectbox("Names (for Pie chart slices):", options=st.session_state.builder_categorical_columns,
                                                               index=st.session_state.builder_categorical_columns.index(st.session_state.builder_names) if st.session_state.builder_names in st.session_state.builder_categorical_columns else 0)
                st.session_state.builder_values = st.selectbox("Values (for Pie chart sizes):", options=st.session_state.builder_numeric_columns,
                                                                index=st.session_state.builder_numeric_columns.index(st.session_state.builder_values) if st.session_state.builder_values in st.session_state.builder_numeric_columns else 0)

            st.session_state.builder_color = st.selectbox("Color by:", options=st.session_state.builder_columns,
                                                          index=st.session_state.builder_columns.index(st.session_state.builder_color) if st.session_state.builder_color in st.session_state.builder_columns else 0)
            
            if st.session_state.builder_chart_type in ["scatter"]:
                st.session_state.builder_size = st.selectbox("Size by (for scatter):", options=st.session_state.builder_numeric_columns,
                                                              index=st.session_state.builder_numeric_columns.index(st.session_state.builder_size) if st.session_state.builder_size in st.session_state.builder_numeric_columns else 0)

        with cols_c2:
            st.session_state.builder_facet_row = st.selectbox("Facet Row by:", options=st.session_state.builder_columns,
                                                               index=st.session_state.builder_columns.index(st.session_state.builder_facet_row) if st.session_state.builder_facet_row in st.session_state.builder_columns else 0)
            st.session_state.builder_facet_col = st.selectbox("Facet Column by:", options=st.session_state.builder_columns,
                                                               index=st.session_state.builder_columns.index(st.session_state.builder_facet_col) if st.session_state.builder_facet_col in st.session_state.builder_columns else 0)
            st.session_state.builder_hover_name = st.selectbox("Hover Name:", options=st.session_state.builder_columns,
                                                                index=st.session_state.builder_columns.index(st.session_state.builder_hover_name) if st.session_state.builder_hover_name in st.session_state.builder_columns else 0)
            
            if st.session_state.builder_chart_type not in ["pie"]:
                st.session_state.builder_log_x = st.checkbox("Logarithmic X-axis", value=st.session_state.builder_log_x)
                if st.session_state.builder_chart_type not in ["histogram"]:
                    st.session_state.builder_log_y = st.checkbox("Logarithmic Y-axis", value=st.session_state.builder_log_y)

            if st.session_state.builder_chart_type == "bar":
                st.session_state.builder_barmode = st.selectbox("Bar Mode:", options=['relative', 'group', 'overlay', 'stack'],
                                                                  index=['relative', 'group', 'overlay', 'stack'].index(st.session_state.builder_barmode) if st.session_state.builder_barmode in ['relative', 'group', 'overlay', 'stack'] else 0)
        ...
        if st.button("📊 Generate Chart", key="btn_generate_custom_chart"):
            ...
            elif st.session_state.builder_chart_type not in ["pie", "histogram"] and (not st.session_state.builder_x_axis or not st.session_state.builder_y_axis):
                st.error("Please select X-axis and Y-axis columns.")
            elif st.session_state.builder_chart_type == "histogram" and not st.session_state.builder_x_axis :
                st.error("Please select X-axis for the histogram.")
            elif st.session_state.builder_chart_type == "pie" and (not st.session_state.builder_names or not st.session_state.builder_values):
                st.error("Please select 'Names' and 'Values' columns for the Pie chart.")
            else:
                try:
                    con = _connect(read_only=True)
                    ...
                    df_full = con.table(f'"{st.session_state.builder_selected_table}"').df() # Load full table first
                    
                    # Apply Knesset Number Filter if applicable
                    if 'KnessetNum' in df_full.columns and st.session_state.ms_knesset_filter:
                        df_full = df_full[df_full['KnessetNum'].isin(st.session_state.ms_knesset_filter)]
                    
                    # Apply Faction Filter if applicable
                    if 'FactionID' in df_full.columns and st.session_state.ms_faction_filter:
                        selected_faction_ids_builder = [faction_display_map[name] for name in st.session_state.ms_faction_filter if name in faction_display_map]
                        if selected_faction_ids_builder:
                             df_full = df_full[df_full['FactionID'].isin(selected_faction_ids_builder)]
                    
                    if df_full.empty:
                        st.warning("No data in the selected table after applying sidebar filters. Cannot generate chart.")
                        ...
                        if st.session_state.builder_x_axis: chart_params["x"] = st.session_state.builder_x_axis
                        if st.session_state.builder_y_axis and st.session_state.builder_chart_type not in ["histogram"]: chart_params["y"] = st.session_state.builder_y_axis
                        if st.session_state.builder_color: chart_params["color"] = st.session_state.builder_color
                        if st.session_state.builder_size and st.session_state.builder_chart_type == "scatter": chart_params["size"] = st.session_state.builder_size
                        if st.session_state.builder_facet_row: chart_params["facet_row"] = st.session_state.builder_facet_row
                        if st.session_state.builder_facet_col: chart_params["facet_col"] = st.session_state.builder_facet_col
                        if st.session_state.builder_hover_name: chart_params["hover_name"] = st.session_state.builder_hover_name
                        if st.session_state.builder_log_x and st.session_state.builder_chart_type not in ["pie"]: chart_params["log_x"] = st.session_state.builder_log_x
                        if st.session_state.builder_log_y and st.session_state.builder_chart_type not in ["pie", "histogram"]: chart_params["log_y"] = st.session_state.builder_log_y
                        if st.session_state.builder_chart_type == "bar" and st.session_state.builder_barmode: chart_params["barmode"] = st.session_state.builder_barmode
                        if st.session_state.builder_chart_type == "pie":
                        ...
                        if st.session_state.builder_chart_type == "bar": fig_builder = px.bar(**chart_params)
                        elif st.session_state.builder_chart_type == "line": fig_builder = px.line(**chart_params)
                        elif st.session_state.builder_chart_type == "scatter": fig_builder = px.scatter(**chart_params)
                        elif st.session_state.builder_chart_type == "pie": fig_builder = px.pie(**chart_params)
                        elif st.session_state.builder_chart_type == "histogram": fig_builder = px.histogram(**chart_params)
                        elif st.session_state.builder_chart_type == "box": fig_builder = px.box(**chart_params)
                        else: st.error(f"Unsupported chart type: {st.session_state.builder_chart_type}"); fig_builder = None
        ...
        if st.button("▶︎ Run Ad-hoc SQL", key="run_adhoc_sql"): 
            if sql_query_input.strip(): 
                try:
                    ...
                    st.error(f"❌ SQL Query Error: {e}"); st.code(str(e) + "\n\n" + _format_exc()) 
