...
app = typer.Typer(name="knesset_cli", help="CLI tool to manage Knesset OData tables.", add_completion=False)
...
def refresh(
    ...
    db_path: Optional[Path] = typer.Option(None, "--db", help="Path to the DuckDB database file."),
):
    """
...
@app.command()
def refresh_factions(db_path: Optional[Path] = typer.Option(None, "--db", help="Path to the DuckDB database file.")):
    """
