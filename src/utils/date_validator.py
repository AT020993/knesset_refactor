            ...
            with get_db_connection(self.db_path, read_only=True, logger_obj=self.logger) as con:
                ...
                if not result3.empty:
                    ...
                    results["first_submission_calculation"] = {
                        ...
                        "bills_with_calculated_dates": (
                            int(row["bills_with_calculated_dates"]) if row["bills_with_calculated_dates"] else 0
                        ),
                        "avg_days_since_first_knesset": (
                            float(row["avg_days_since_first_knesset"]) if row["avg_days_since_first_knesset"] else 0.0
                        ),
                    }
                ...
                results["validation_passed"] = all(
                    [
                        ...
                        results.get("chronological_consistency", {}).get("committee_before_initiator", 1)
                        < 10,  # Allow some tolerance
