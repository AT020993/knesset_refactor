...
class AggregationValidator:
            ...
            with get_db_connection(self.db_path, read_only=True, logger_obj=self.logger) as con:
                ...
                if not result2.empty:
                    ...
                    results["faction_bill_counting_validation"] = {
                        ...
                        "total_count_difference": (
                            int(row["total_count_difference"]) if row["total_count_difference"] else 0
                        ),
                    }
                ...
                if not result3.empty:
                    ...
                    results["node_sizing_validation"] = {
                        ...
                        "mks_with_collaborations": (
                            int(row["mks_with_collaborations"]) if row["mks_with_collaborations"] else 0
                        ),
                    }
            ...
            with get_db_connection(self.db_path, read_only=True, logger_obj=self.logger) as con:
                ...
                if not result2.empty:
                    ...
                    results["bill_initiator_consistency"] = {
                        ...
                        "total_bills_by_faction": (
                            int(row["total_bills_by_faction"]) if row["total_bills_by_faction"] else 0
                        ),
                    }
                ...
                if not result3.empty:
                    ...
                    results["distribution_totals_validation"] = {
                        "total_bills_by_status": (
                            int(row["total_bills_by_status"]) if row["total_bills_by_status"] else 0
                        ),
                ...
                results["validation_passed"] = all(
                    [
                        ...
                        abs(
                            results.get("distribution_totals_validation", {}).get("total_bills_by_status", 0)
                            - results.get("distribution_totals_validation", {}).get("actual_total_bills", 1)
                        )
                        <= 1,  # Allow small rounding differences
                    ]
                )
    ...
    def validate_time_series_aggregations(self) -> Dict[str, Any]:
        ...
        try:
            with get_db_connection(self.db_path, read_only=True, logger_obj=self.logger) as con:
                ...
                results["validation_status"] = "completed"
                results["validation_passed"] = results.get("time_series_consistency", {}).get(
    ...
    def run_comprehensive_aggregation_validation(self) -> Dict[str, Any]:
        ...
        if not validation_results["network_chart_validation"].get("validation_passed", False):
            validation_results["summary"]["aggregation_issues"].append(
                "Network chart aggregation inconsistencies detected"
            )
        ...
        if not validation_results["time_series_validation"].get("validation_passed", False):
            validation_results["summary"]["aggregation_issues"].append(
                "Time series aggregation inconsistencies detected"
            )
