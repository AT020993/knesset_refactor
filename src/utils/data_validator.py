        ...
        try:
            with get_db_connection(self.db_path, read_only=True, logger_obj=self.logger) as con:
                ...
                if not result1.empty:
                    results['bills_without_primary_initiators'] = int(result1.iloc[0]['bills_without_primary_initiators'])
                ...
                SELECT
                    ...
                    COUNT(DISTINCT CASE WHEN bi.Ordinal > 1 THEN bi.PersonID END) as actual_supporting_count
                FROM KNS_Bill b
                ...
                FROM (
                    SELECT
                        b.BillID,
                        COUNT(DISTINCT CASE WHEN bi.Ordinal > 1 THEN bi.PersonID END) as supporting_count,
                ...
                if not result5.empty:
                    results['bills_with_supporting_count_errors'] = int(result5.iloc[0]['bills_with_supporting_count_errors'])
    ...
    def validate_committee_join_consistency(self) -> Dict[str, Any]:
        ...
        try:
            with get_db_connection(self.db_path, read_only=True, logger_obj=self.logger) as con:
                ...
                results['validation_status'] = 'completed'
                results['validation_passed'] = results.get('committee_join_stats', {}).get('join_success_rate', 0) > 70.0
