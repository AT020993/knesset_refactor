...
class DataQualityDashboard:
    ...
    def _create_overview_tab(self) -> None:
        ...
        with col1:
            st.metric(
                "Database Status",
                "✅ Connected" if self.db_path.exists() else "❌ Not Found",
                delta=None
            )
    ...
    def _create_diagnostic_tools_tab(self) -> None:
        ...
        query_input = st.text_area(
            "Enter SQL Query",
            height=150,
            placeholder="SELECT * FROM KNS_Bill LIMIT 10;"
        )
    ...
    def _display_bill_initiator_results(self, results: Dict[str, Any]) -> None:
        ...
        with col1:
            st.metric(
                "Bills without Primary Initiators",
                results.get('bills_without_primary_initiators', 0)
            )
        
        with col2:
            st.metric(
                "Bills with Multiple Primaries",
                results.get('bills_with_multiple_primaries', 0)
            )
        
        with col3:
            st.metric(
                "Counting Inconsistencies",
                results.get('bills_with_counting_inconsistencies', 0)
            )
    ...
    def _display_committee_join_results(self, results: Dict[str, Any]) -> None:
        ...
        if join_stats:
            ...
            with col1:
                st.metric(
                    "Bills with Committee ID",
                    join_stats.get('bills_with_committee_id', 0)
                )
            
            with col2:
                st.metric(
                    "Successful Joins",
                    join_stats.get('successful_joins', 0)
                )
            
            with col3:
                st.metric(
                    "Join Success Rate",
                    f"{join_stats.get('join_success_rate', 0):.1f}%"
                )
    ...
    def _display_date_results(self, results: Dict[str, Any], date_type: str) -> None:
        ...
        for key, value in results.items():
            if isinstance(value, dict) and any(word in key.lower() for word in ['date', 'chronological', 'consistency']):
    ...
    def _display_comprehensive_results(self, results: Dict[str, Any], validation_type: str) -> None:
        ...
        # Issues list
        issues = summary.get('critical_issues', []) or summary.get('aggregation_issues', []) or summary.get('date_issues', [])
        if issues:
