...
class SecureQueryBuilder:
    ...
    def build_filter_condition(
        self, column: str, operator: FilterOperator, value: Any = None, values: Optional[List[Any]] = None
    ) -> str:
        """
    ...
    def build_knesset_filter(
        self, knesset_numbers: Optional[List[int]], column: str = "KnessetNum"
    ) -> Tuple[str, bool]:
        """
        ...
        if len(knesset_numbers) == 1:
            condition = self.build_filter_condition(column, FilterOperator.EQUALS, knesset_numbers[0])
            return condition, True
        else:
            condition = self.build_filter_condition(column, FilterOperator.IN, values=knesset_numbers)
            return condition, False

    def build_faction_filter(self, faction_names: Optional[List[str]], column: str = "FactionName") -> str:
        """
        ...
        return self.build_filter_condition(column, FilterOperator.IN, values=faction_names)

    def build_date_range_filter(
        self, start_date: Optional[str], end_date: Optional[str], column: str = "SubmitDate"
    ) -> str:
        """
        ...
        if start_date:
            conditions.append(self.build_filter_condition(column, FilterOperator.GREATER_EQUAL, start_date))

        if end_date:
            conditions.append(self.build_filter_condition(f"date({column})", FilterOperator.LESS_EQUAL, end_date))
    ...
    def build_advanced_filters(self, filters: Dict[str, Any], table_prefix: str = "") -> str:
        """
        ...
        if filters.get("query_type_filter"):
            conditions.append(
                self.build_filter_condition(f"{prefix}TypeDesc", FilterOperator.IN, values=filters["query_type_filter"])
            )

        # Status filters using joined table alias
        if filters.get("query_status_filter"):
            conditions.append(
                self.build_filter_condition('s."Desc"', FilterOperator.IN, values=filters["query_status_filter"])
            )
        ...
        if filters.get("session_type_filter"):
            conditions.append(
                self.build_filter_condition(
                    f"{prefix}SubTypeDesc", FilterOperator.IN, values=filters["session_type_filter"]
                )
            )
        ...
        if filters.get("bill_type_filter"):
            conditions.append(
                self.build_filter_condition(
                    f"{prefix}SubTypeDesc", FilterOperator.IN, values=filters["bill_type_filter"]
                )
            )
        ...
        if filters.get("bill_status_filter"):
            conditions.append(
                self.build_filter_condition('s."Desc"', FilterOperator.IN, values=filters["bill_status_filter"])
            )
        ...
        if filters.get("start_date"):
            conditions.append(
                self.build_filter_condition(f"{prefix}SubmitDate", FilterOperator.GREATER_EQUAL, filters["start_date"])
            )

        if filters.get("end_date"):
            conditions.append(
                self.build_filter_condition(f"date({prefix}SubmitDate)", FilterOperator.LESS_EQUAL, filters["end_date"])
            )
    ...
    def build_time_series_query(
        ...
        conditions.append(
            builder.build_filter_condition(
                f"CAST({date_column} AS TIMESTAMP)", FilterOperator.GREATER_EQUAL, "1949-01-25"  # First Knesset date
            )
        )

        conditions.append(
            builder.build_filter_condition(
                f"CAST({date_column} AS TIMESTAMP)", FilterOperator.LESS_EQUAL, "CURRENT_DATE + INTERVAL '1 year'"
            )
        )
...
def build_pagination_clause(page: int, page_size: int, builder: SecureQueryBuilder) -> str:
    """
