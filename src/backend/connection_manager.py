        ...
        with self._lock:
            for conn_id, info in list(self._active_connections.items()):
                if info['weakref'] is weakref_obj:
                    self._logger.warning(f"Connection {conn_id} to {info['db_path']} was garbage collected without explicit close()")
                    del self._active_connections[conn_id]
                    break
    ...
    def log_connection_stats(self) -> None:
        ...
        if stats:
            ...
            for conn_id, info in stats.items():
                ...
                self._logger.warning(f"  Connection {conn_id}: {info['db_path']}, age: {age:.1f}s, thread: {info['thread_id']}")
        else:
...
def connect_db(db_path: Path, read_only: bool = True, _logger_obj: logging.Logger | None = None) -> duckdb.DuckDBPyConnection:
    ...
    _logger_obj.warning("connect_db() is deprecated. Use get_db_connection() context manager to prevent connection leaks.")
